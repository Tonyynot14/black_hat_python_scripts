...
##### List of hex characters, characters that are 3 connical characters long, some are 6 when not printable such as \x00 
##### Is three for printable because they have quotes in the repr like this "'A" for repr(chr(65))
HEX_FILTER = ''.join(
...
def proxy_handler(client_socket,remote_host,remote_port,receive_first):
    ...
    while True:
        ...
        if len(local_buffer):
            line = "[==>] Received %d from localhost" % len(local_buffer)
            print(line)
            hexdump(line)
...
def main():
    if len(sys.argv[1:]) != 5:
        print("Usage: ./proxy.py [localhost] [local port] [remotehost] [remoteport] [receive_first]")
